...
{
    public class EmployeeOpportunities : Controller
    {
        ...
        public EmployeeOpportunities(IConfiguration configuration, IApiRepository apiRepository, IWebHostEnvironment hostEnvironment)
        {
        ...
        [HttpPost]
        public async Task<string> AddorEditEmployeeRoles([FromBody] EmployeeOpenRole model)
        {
            try
            {
                var date = DateTime.Now;
                model.CreatedDateTime = date;
                var RoleEditResponse = await _apiRepository.MakeApiCallAsync("api/Admin/AddorEditEmployeeRoles", HttpMethod.Post, model);
        ...
        public async Task<string> DeleteEmployeeRole([FromBody] EmployeeOpenRole rolesModel)
        {
            try
            {
                if (rolesModel != null)
                {
                    var rolesData = await _apiRepository.MakeApiCallAsync("api/Admin/DeleteEmployeeRolesById", HttpMethod.Post, rolesModel);
                    return rolesData;
                }
                else
                {
        ...
        [HttpPost]
        public async Task<string> GetRolesdataById([FromBody] EmployeeOpenRole model)
        {
            var rolesList = await _apiRepository.MakeApiCallAsync("api/Admin/GetEmployeeRolesDataById", HttpMethod.Post, model);
            return rolesList;
        }
